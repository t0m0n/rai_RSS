...
def resolve_final_mp3_url(indirect_url):
    try:
        ...
        response = session.get(indirect_url, allow_redirects=True, timeout=10)
        if response.status_code == 200:
            # Look for the direct .mp3 or audio link in final URL or content
            # Sometimes it's in the response.url, sometimes inside HTML
            ...
            for audio_tag in soup.find_all('audio'):
                ...
                if src and src.endswith(".mp3"):
                    return src
            # fallback: try to find .mp3 with regex
